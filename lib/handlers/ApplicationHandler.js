...
exports.handleGetCallParticipants = function (req, res) {
    ...
    if (Name && ConferenceId) {
        twilio.conferences(ConferenceId).participants.list(function(err, data) {
            if (!err) {
                console.log("raw twilio data: " + JSON.stringify(data));
                parseParticipants(data.participants, function(participantNumberList) {
                    console.log("return from parse: " + participantNumberList);
                    res.status(200);
...
function parseParticipants(participants, callback) {
    var twilio = require('twilio')(nconf.get('TWILIO_ACCOUNT_SID'), nconf.get('TWILIO_AUTH_TOKEN'));
    var promises = [];
